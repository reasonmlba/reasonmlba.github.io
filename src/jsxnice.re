  ...
  let createElement =
    ...
    Rereact.element({
      ...
      render: (_) => {
        ...
        Rereact.Nested(
          element,
          {id, value, disabled, onClick, src, className: Some(styles), onChange},
          children
        );
      }
    });
...
/* module Block = {
     let createElement = Box.createElement(~display=Block);
   }; */
module Inline = {
  let createElement = Box.createElement(~display=Inline);
};

module InlineBlock = {
  let createElement = Box.createElement(~display=InlineBlock);
};

module Row = {
  let createElement = Box.createElement(~display=Flex, ~flexDirection=Row);
};

module Col = {
  let createElement = Box.createElement(~display=Flex, ~flexDirection=Column);
};
/*
 module Grid = {
